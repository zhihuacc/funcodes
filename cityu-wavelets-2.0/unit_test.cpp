...
//			fs_param.md_fs_param_at_level[i].oned_fs_param_at_dim[j].opt = "sincos";
	...
	ML_MC_Coefs_Set coefs_set;
	ML_MC_Filter_Norms_Set norms_set;
...
//			fs_param.md_fs_param_at_level[i].oned_fs_param_at_dim[j].opt = "sincos";
	...
	for (int i = 0; i < nd; ++i)
	{
		border[i] = ((mat.size[i] + 32) & (~((1 << nlevels) - 1))) - mat.size[i];
	}

//	mat_border_extension<double>(mat, border, "sym", mat_ext);
	mat_ext = mat;
...
int Unit_Test::construct_1d_filter_test(int argc, char **argv)
{
	...
	linspace<double>(complex<double>(-M_PI, 0), complex<double>(M_PI, 0), 512, x_pts);
	cout << "X: " << endl;
...
#define mat_select_test_FLOAT_TYPE float
	...
	mat_subfill<mat_select_test_FLOAT_TYPE>(zeros, index_at_dim, sub_mat, zeros);
