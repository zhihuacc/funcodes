...
{
    float *obd = static_cast<float*>(observed_data);
	return 0.1f * min(obd[labels[0]], 15.0f);
}

float smoothness_cost(const vector<int> &labels, void *observed_data)
{
    return min(static_cast<float>(abs(labels[0] - labels[1])), 3.0f);
}
...
void free_observed_data_destructor(void *observed_data)
{
	...
	float *m = static_cast<float*>(observed_data);
	delete [] m;
	return;
}
...
factor_graph_(rows_ * cols_, 3 * rows_ * cols_)
{
    ...
    for (int y = 0; y < rows_; y++)
    {
        for (int x = 0; x < cols_; x++)
        {
            ...
            float *obd_row = new float[LABEL_SET_SIZE];
            memcpy(obd_row, observed_data[loc], sizeof(float) * LABEL_SET_SIZE);
